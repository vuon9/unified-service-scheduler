...
> Backend REST API for vehicle service scheduling
...
- **Note**: The frontend pins dealership to `d1` (Saigon Auto). The backend supports multiple dealerships via seed data, but there is no UI for switching dealerships. Technicians and bays are filtered by the currently selected dealership. This was intentionally kept simple for the demo — the API is dealership-aware from day one.
